...
import {d18i, d18s} from "../data";

function q1() {
    const cubes = splitLinesIntoArray(d18i).map(cube => cube.split(',').map(Number));
    ...
    console.log("d18q1: " + surfaceArea);
}

function q2() {
    const cubesAsString = splitLinesIntoArray(d18i);
    ...
    console.log("d18q2: " + exteriorSurfaceArea);
}

function getPossibleAdjacents(cube: number[]) {
    const [x, y, z] = cube;
    return [
        [x - 1, y, z],
        [x + 1, y, z],
