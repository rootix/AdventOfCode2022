...
function q1() {
    const blueprints = parseBlueprints(d19i);
    const qualityLevel = sum(blueprints.map(b => getMaxGeodes(b, 24) * b.id));

    console.log("d19q1: " + qualityLevel);
}

function q2() {
    const blueprints = parseBlueprints(d19i).slice(0, 3);
    const geodeMultiplication = multiply(blueprints.map(b => getMaxGeodes(b, 32)));

    console.log("d19q2: " + geodeMultiplication);
}

function parseBlueprints(input: string): Blueprint[] {
    return splitLinesIntoArray(input)
...
function getMaxGeodes(blueprint: Blueprint, remaining: number): number {
...
function optimizeStateForCache(state: State, maxOreCosts: number, maxClayCosts: number, maxObsidianCosts: number) {
