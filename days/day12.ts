...
function q1() {
    const {start, end, map} = parseHeightMap(d12i);
    console.log("d12q1: " + hikeToE(map, [start], end));
}

function q2() {
    const {end, map} = parseHeightMap(d12i);
    console.log("d12q2: " + hikeToE(map, getStarts(map), end));
}

function parseHeightMap(input: string): { start: Position, end: Position, map: number[][] } {
    let start: Position;
    let end: Position;
...
function hikeToE(map: number[][], starts: Position[], end: Position) {
