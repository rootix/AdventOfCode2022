import {mod, splitLinesByEmptyLines, splitLinesIntoArray} from '../utils';
import {d22i, d22s} from '../data';
...
const [mapPart, instructionPart] = splitLinesByEmptyLines(d22i);
...
function q1() {
    ...
    console.log("d22q1: " + followInstructions(pos, false));
}

function q2() {
    const pos: Position = {row: 0, col: xEdges[0].min, face: 1, facing: Direction.RIGHT};
    console.log("d22q2: " + followInstructions(pos, true));
}

function followInstructions(pos: Position, cubic: boolean) {
...
function turn(pos: Position, direction: 'R' | 'L') {
    pos.facing = direction === 'R' ? mod(pos.facing + 1, 4) : mod(pos.facing - 1, 4);
}

function move(pos: Position, times: number, cubic: boolean) {
...
function wrapOrStay(pos: Position) {
...
function changeFaceOrStay(currPos: Position, newPos: Position) {
